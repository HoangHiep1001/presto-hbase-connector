...
//        log.info("\nPresto HBase Connector Config：" + this.config.toString());
    ...
    public ConnectorSplitSource getSplits(
            ...
            ConnectorTableHandle tables,
            SplitSchedulingStrategy splitSchedulingStrategy,
            DynamicFilter dynamicFilter) {
        HBaseTableHandle tableHandle = (HBaseTableHandle) tables;
...
//                + " finished, using " + (System.currentTimeMillis() - start) + " mill seconds.");
    ...
    private List<HBaseSplit> getSplitsForScan(List<ConditionInfo> conditions,
        ...
        if (!notSaltyPartStartKeyList.isEmpty()) {
            ...
            if (config.isSeperateSaltPart()) {
                ...
                if (saltyPartStartKeyList.size() * notSaltyPartStartKeyList.size() <= Constant.MAX_SPLIT_COUNT) {
                    for (String notSaltyPartStartKey : notSaltyPartStartKeyList) {
                        for (StartAndEnd saltyPartStartKey : saltyPartStartKeyList) {
                            String finalStartKey = saltyPartStartKey.start + Constant.ROWKEY_SPLITER + notSaltyPartStartKey + Constant.ROWKEY_SPLITER;
                            splits.add(createHBaseSplit(schemaName, tableName,
                                    tableMetaInfo.getRowKeyColName(), hostIndex,
                                    finalStartKey, finalStartKey + Constant.ROWKEY_TAIL, conditions,
                                    -1, null, null));
                            hostIndex += 1;
                        }
                    }
            ...
            // there is no seperate salty part, we have to take notSaltyPartStartKeyList as startKey and stopKey
                ...
                for (String notSaltyPartStartKey : notSaltyPartStartKeyList) {
                    splits.add(createHBaseSplit(schemaName, tableName,
                            tableMetaInfo.getRowKeyColName(), hostIndex,
                            notSaltyPartStartKey + Constant.ROWKEY_SPLITER,
                            notSaltyPartStartKey + Constant.ROWKEY_SPLITER + Constant.ROWKEY_TAIL,
    ...
    private void addSplitsOnlyBySaltyPart(List<HBaseSplit> splits, String schemaName, String tableName,
        ...
        List<StartAndEnd> startAndEndRowKeys =
                getSaltyParts(rowKeyFirstCharRange, Constant.ROWKEY_PREFIX_SPLIT_COUNT);
        for (StartAndEnd range : startAndEndRowKeys) {
            splits.add(createHBaseSplit(schemaName, tableName,
                    rowKeyColName, hostIndex,
                    range.start + "", range.end + Constant.ROWKEY_TAIL, conditions,
                    -1, null, null));
            hostIndex += 1;
        }
    }
    ...
    private List<StartAndEnd> getSaltyParts(String rowKeyFirstCharRange, int saltCount) {
        List<StartAndEnd> prefixRanges = Arrays.stream(rowKeyFirstCharRange.split(Constant.COMMA))
    ...
    class StartAndEnd {
        ...
        StartAndEnd(String startAndEnd) {
            String[] se = startAndEnd.split(Constant.SWUNG_DASH);
    ...
    private List<HBaseSplit> getSplitsForBatchGet(List<ConditionInfo> conditions,
        ...
        if (rowKeys.size() / Constant.BATCHGET_SPLIT_RECORD_COUNT > Constant.BATCHGET_SPLIT_MAX_COUNT) {
            maxSplitSize = rowKeys.size() / Constant.BATCHGET_SPLIT_MAX_COUNT;
        } else {
            maxSplitSize = Constant.BATCHGET_SPLIT_RECORD_COUNT;
        }
        for (ConditionInfo cond : rowKeys) {
            ...
            if (splitConditions.size() >=
                    maxSplitSize + (splits.size() < rowKeys.size() % Constant.BATCHGET_SPLIT_RECORD_COUNT ? 1 : 0)) {
    ...
    private List<String> getScanStartKey(List<ConditionInfo> conditions,
        ...
        List<ConditionInfo> formatCondition = conditions.stream()
                ...
                .filter(condition -> condition.getColName().equals(rowKeyFormat[formatIndex])
                        && condition.getOperator() == Constant.CONDITION_OPER.EQ)
                .collect(Collectors.toList());
    ...
    private List<ConditionInfo> findConditionFromConstraint(TupleDomain<ColumnHandle> constraint) {
        ...
        for (ColumnHandle ch : keySet) {
            ...
            String columnName = Utils.mapField.get(hch.getColumnName());
            ...
            if (domain.isSingleValue()) {
                ...
                if (value instanceof Slice) {
                    handles.add(new ConditionInfo(columnName, Constant.CONDITION_OPER.EQ,
                    ...
                    if (range.isSingleValue()) {
                        ...
                        if (!range.isLowUnbounded()) {
                            if (range.isLowInclusive()) {
                                // >=
                                handles.add(new ConditionInfo(columnName, Constant.CONDITION_OPER.GE,
                                        range.getLowValue(), domain.getType()));
                            } else {
                                // >
                                handles.add(new ConditionInfo(columnName, Constant.CONDITION_OPER.GT,
                                        range.getLowValue(), domain.getType()));
                        ...
                        if (!range.isHighUnbounded()) {
                            if (range.isHighInclusive()) {
                                // <=
                                handles.add(new ConditionInfo(columnName, Constant.CONDITION_OPER.LE,
                                        range.getHighValue(), domain.getType()));
                            } else {
                                // <
                                handles.add(new ConditionInfo(columnName, Constant.CONDITION_OPER.LT,
                                        range.getHighValue(), domain.getType()));
                            }
                        }
                    }
