        ...
        List<Get> gets = rowKeys.stream().map(rowKey -> {
                    ...
                    for (ColumnHandle ch : columnHandles) {
                        ...
                        if (this.split.getRowKeyName() != null
                                && this.split.getRowKeyName().equals(columnName)) {
                            continue;
                        }
                        get.addColumn(Bytes.toBytes(hch.getFamily()), Bytes.toBytes(columnName));
                    }
                    return get;
                }
            ...
            if (results == null) {
                ...
                for (Cell cell : record.rawCells()) {
                    colName = Bytes.toString(
                            Utils.arrayCopy(cell.getQualifierArray(), cell.getQualifierOffset(), cell.getQualifierLength()));
