...
public class HBaseRecordCursor implements RecordCursor {
    ...
    public boolean advanceNextPosition() {
        throw new UnsupportedOperationException("loi vl :(");
    }
    ...
    Object matchValue(Type type, byte[] value) {
        ...
        if (value.length == 0 && !typeClass.equals(Constant.VARCHAR_CLASS)) {
            return null;
        }

        if (typeClass.equals(Constant.VARCHAR_CLASS)) {
            return Bytes.toString(value);
        } else if (typeClass.equals(Constant.INTEGER_CLASS)) {
            return Bytes.toInt(value);
        } else if (typeClass.equals(Constant.BIGINT_CLASS)) {
            return Bytes.toLong(value);
        } else if (typeClass.equals(Constant.DOUBLE_CLASS)) {
            return Bytes.toDouble(value);
        } else if (typeClass.equals(Constant.TIMESTAMP_CLASS)) {
            return Bytes.toLong(value);
        } else if (typeClass.equals(Constant.BOOLEAN_CLASS)) {
            // 0: false, 1: true
            return Bytes.toInt(value);
    ...
    public double getDouble(int field) {
        this.checkFieldType(field, DoubleType.DOUBLE);
    ...
    public Slice getSlice(int field) {
        ...
        if (type instanceof VarcharType) {
            ...
            return Slices.utf8Slice(String.valueOf(Decimals.encodeScaledValue(dec, decimalType.getScale())));
        } else {
    ...
    void setRowKeyValue2FieldsAry(Result record, int fieldIndex) {
        ...
        if (fieldIndexMap.containsKey(this.rowKeyColName.hashCode())) {
            if (record.rawCells() != null && record.rawCells().length > 0) {
                ...
                String rowKeyValue = DatatypeConverter.printHexBinary(
                        Utils.arrayCopy(cell.getRowArray(), cell.getRowOffset(), cell.getRowLength()));
