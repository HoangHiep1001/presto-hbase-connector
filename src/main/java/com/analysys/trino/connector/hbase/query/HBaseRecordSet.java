...
public class HBaseRecordSet implements RecordSet {
    ...
    public Map<String, String> initMapField() {
        return Utils.getMapField(hBaseSplit.getSchemaName(), hBaseSplit.getTableName(), config.getMetaDir());
    }
    ...
    private String getFamilyByColumnName(String columnName, List<HBaseColumnHandle> columns) {
        ...
        HBaseColumnHandle column = columns.stream()
                .filter(col -> columnName.equals(this.mapField.get(col.getColumnName()))).findAny().orElse(null);
        if (column != null) {
    ...
    private Scan getScanFromPrestoConstraint() {
        ...
        this.columnHandles.forEach(hch -> {
            if (this.hBaseSplit.getRowKeyName() == null) {
                scan.addColumn(
                        Bytes.toBytes(hch.getFamily()), Bytes.toBytes(this.mapField.get(hch.getColumnName())));
            } else {
                if (!this.hBaseSplit.getRowKeyName().equals(this.mapField.get(hch.getColumnName()))) {
                    scan.addColumn(
                            Bytes.toBytes(hch.getFamily()), Bytes.toBytes(this.mapField.get(hch.getColumnName())));
                }
            }
    ...
    private void initFieldIndexMap(List<HBaseColumnHandle> columnHandles) {
        if (this.mapField.size() == 0) {
            this.mapField = initMapField();
        }
        columnHandles.forEach(hch -> {
            fieldIndexMap.put(this.mapField.get(hch.getColumnName()).hashCode(), hch);
        });
    }
